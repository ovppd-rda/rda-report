...
- Daughters (23%, n=96) and sons (22%, n=90) are the most common co-occupants, together accounting for nearly half of all responses.

- Wives (9%, n=37), husbands (6%, n=25), and those identified simply as “spouse” (5%, n=22) form a notable but smaller group.

- Co-heads (4%, n=18), children (4%, n=18), nieces (4%, n=17), and various extended relatives (e.g., brothers, nephews, cousins) each represent small shares (2–4%).

- Categories such as sister-in-law, sister, mother-in-law, daughter-in-law, live-in partner, granddaughter, and others each account for only 1–2% of responses. Father, uncle, relative, and “none” were reported by only 2 respondents each.

```{r}

title_plot <- glue("Co-occupant relationship to unit owner (n = {nrow(housing_dta)})")

subtitle_text <- str_wrap("Daughters and sons make up the largest share of co-occupants, while spouses and extended relatives represent smaller proportions.", 70)


p_co_occupant <- 
    co_occupant_dta |> 
    select(relationship_to_the_head) |> 
    mutate(relationship_to_the_head = str_to_lower(relationship_to_the_head)) |>
    na.omit() |> 
    mutate(relationship_to_the_head = case_when(
    str_detect(relationship_to_the_head, "adopted sister") ~ "sister",
    str_detect(relationship_to_the_head, "stepdaughter") ~ "daughter",
    str_detect(relationship_to_the_head, "niece") ~ "niece",
    str_detect(relationship_to_the_head, "son") ~ "son",
    str_detect(relationship_to_the_head, "officemate|colleague|friend|employer|housemate") ~ "colleagues",
    str_detect(relationship_to_the_head, "head") ~ "co-head",
    str_detect(relationship_to_the_head, "scholar|student") ~ "working/scholar student",
    str_detect(relationship_to_the_head, "sister-in") ~ "sister-in-law",
    str_detect(relationship_to_the_head, "sibling") ~ "child",
    str_detect(relationship_to_the_head, "no") ~ "none",
    TRUE ~ relationship_to_the_head
  )) |> 
  plt_barplot(variable = relationship_to_the_head, title_plot) +
  scale_x_continuous(labels = percent_format(), limits = c(0, 0.50), breaks = seq(0, 0.5, 0.10)) 

## save plot
ggsave(
    plot = p_co_occupant,
    filename = "plot/co_occupant.jpeg",
    units = "in",
    width = 9,
    height = 8,
    dpi = 300
...
# display plot
knitr::include_graphics("plot/co_occupant.jpeg")
```

### Number of vehicles owned {.unnumbered}

- The majority of respondents reported having a garage (80%), and within this group motorcycles are the most common vehicle (36%, n=96).

- Cars (14%, n=36) and SUVs (9%, n=25) are more frequently owned by those with garages, alongside smaller shares of vans/MPVs and pick-ups (both 4%).

- Trucks (2%, n=5), bicycles (2%, n=5), and tricycles (0%, n=1) were reported in smaller numbers among garage owners.

- Motorcycles remain the leading vehicle type (9%, n=25), followed by cars (5%, n=12) and trucks (3%, n=7).

- SUVs (1%, n=2), tricycles (1%, n=3), and single cases of vans, pick-ups, and bicycles highlight limited diversity in vehicle types among households without garages.

```{r}

title_plot <- glue("Vehicle owned and garage availability (n = {nrow(housing_dta)})")

subtitle_text <- str_wrap("Motorcycles are the most common vehicles owned, especially among households with garages, while cars and trucks appear more frequently among those without garages.", 70)

vehicles_garage_dta <- 
  co_occupant_dta |> 
  select(with_garage_y_n, vehicle_type) |> 
  mutate(with_garage_y_n = str_to_lower(with_garage_y_n)) |> 
  mutate(with_garage = if_else(with_garage_y_n == "yes", "with garage (80%)", "without garage (20%)")) |> 
  na.omit() |>
  mutate(
    vehicle_category = case_when(
      str_detect(vehicle_type, regex("motor|scooter|2 wheel|burgman|click", ignore_case = TRUE)) ~ "Motorcycle",
      str_detect(vehicle_type, regex("3 wheel|tricycle|pedicab", ignore_case = TRUE)) ~ "Tricycle",
      str_detect(vehicle_type, regex("sedan|hatchback|car|ecosport|innova|alto|surf|mux|vios|ranger|wigo|eon", ignore_case = TRUE)) ~ "Car",
      str_detect(vehicle_type, regex("suv|crossover", ignore_case = TRUE)) ~ "SUV",
      str_detect(vehicle_type, regex("van|mpv|urvan|grandia", ignore_case = TRUE)) ~ "Van/MPV",
      str_detect(vehicle_type, regex("pick|hilux|strada|frontier", ignore_case = TRUE)) ~ "Pick-up",
      str_detect(vehicle_type, regex("truck|cab", ignore_case = TRUE)) ~ "Truck",
      str_detect(vehicle_type, regex("bicycle", ignore_case = TRUE)) ~ "Bicycle",
      TRUE ~ "Other"
    )
  ) |>
  count(with_garage, vehicle_category, sort = TRUE) |> 
  n_pct() |> 
  mutate(vehicle_category = reorder_within(vehicle_category, pct, with_garage))

p_vehicle_garage <- 
  vehicles_garage_dta |> 
  ggplot(aes(pct, vehicle_category, fill = with_garage)) +
  facet_wrap(~ with_garage, scales = "free", ncol = 1) +
  geom_col(width = 0.7, show.legend = F) +
  geom_text(aes(label = pct_lab), hjust = -0.1, fontface = "bold", size = 4) +
  scale_x_continuous(labels = percent_format(), limits = c(0, 0.75), breaks = seq(0, 0.75, 0.25)) +
  scale_y_reordered() +
...
## save plot
ggsave(
    plot = p_vehicle_garage,
    filename = "plot/vehicle_garage.jpeg",
    units = "in",
    width = 8,
    height = 8,
    dpi = 300
...
# display plot
knitr::include_graphics("plot/vehicle_garage.jpeg")
```


### Number of pets owned {.unnumbered}

- Nearly three-fourths of respondents reported owning dogs (74%, count: 212), with an average of 3 per household and a wide range (1–17).

- About 15% of respondents own cats (count: 42), averaging nearly 3 per household, with some owning up to 12.

- Though only 10% of respondents own fish (count: 30), each household consistently reported 30 fish, showing high volume but limited distribution.

- Very few respondents own birds (0.7%, count: 2), with ownership limited to exactly 2 per household. Only one respondent reported owning chickens (0.35%, count: 1).


::: {.callout-warning}
## For verification:
The reported number of dogs appears unusually high. Some respondents indicated owning up to 17 dogs. This needs verification to ensure it is not a typographical error.
:::

```{r}
title_plot <- glue("Type and number of pets owned (n = {nrow(housing_dta)})")

subtitle_text <- str_wrap("Dogs are the most commonly owned pets, followed by cats and fish, while birds and chickens are rarely reported.", 70)


pets_owned_dta <- 
  co_occupant_dta |> 
  select(type_of_pet, total_number_of_pets_owned) |> 
  mutate(pet_type = case_when(
    str_detect(type_of_pet, "Cat|Kitten") ~ "Cats",
    str_detect(type_of_pet, "Dog|Canine|Puppy") ~ "Dogs",
    str_detect(type_of_pet, "Chick|Rooster") ~ "Chickens",
    str_detect(type_of_pet, "Bird") ~ "Birds",
    str_detect(type_of_pet, "Fish") ~ "Fish",
    TRUE ~ type_of_pet
  )) |> 
  mutate(no_pets = str_extract(total_number_of_pets_owned, "\\d+")) |> 
  mutate(no_pets = as.integer(no_pets)) |> 
  select(pet_type, no_pets) |> 
  na.omit() |> 
  group_by(pet_type) |> 
  summarise(
    count = sum(no_pets),
    avg = mean(no_pets, na.rm = TRUE),
    min = min(no_pets),
    max = max(no_pets)
  ) |> 
  mutate(pct = count/sum(count)) |> 
  mutate(pct_lab = str_c(round(pct * 100, 2), "% ", "(count: ", count, "; avg: ", round(avg, 0), "; min-max [", min, "-", max, "])")) |> 
  mutate(pet_type = fct_reorder(pet_type, pct))

p_pets_owned <- 
  pets_owned_dta |> 
  ggplot(aes(pct, pet_type)) +
    geom_col(width = 0.6) +
    geom_text(aes(label = pct_lab), hjust = -0.05, fontface = "bold", size = 4) +
    scale_x_continuous(labels = percent_format(), limits = c(0, 1.50), breaks = seq(0, 1, 0.25)) +
    coord_cartesian(expand = TRUE, clip = "off") +
    labs(
        title = title_plot,
        subtitle = subtitle_text,
...
# display plot
knitr::include_graphics("plot/pets_owned.jpeg")
```


### Pet's status of vaccination

- Vaccinations were very low from 2020 to 2024, with only 1–3 cases recorded each year. A dramatic increase occurred in 2025, with 52 vaccinations — the highest across all years. Currently, 22 vaccination were reported in 2026.


```{r}
title_plot <- glue("Yearly pet vaccination (n = {nrow(housing_dta)})")
subtitle_text <- str_wrap("Pet vaccinations remained low in earlier years, spiked sharply in 2025.", 80)

vacc_date_dta <- 
  co_occupant_dta |> 
  select(date_of_last_vaccination) |> 
  mutate(vacc_date = str_extract(date_of_last_vaccination, "\\d+")) |> 
  mutate(vacc_date = as_date(as.numeric(vacc_date), origin = "1899-12-30")) |> 
  mutate(year_month = floor_date(vacc_date, "year")) |> 
  mutate(year_month = case_when(
      year(year_month) == 1900 ~ as.Date("2026-01-01"),
      year(year_month) == 1905 ~ as.Date("2025-01-01"),
      TRUE ~ year_month)) |> 
  group_by(year_month) |> 
  summarise(count = n(), .groups = "drop") |> 
  na.omit()
...
p_pet_vaccination <- 
    ggplot(vacc_date_dta, aes(x = year_month, y = count)) +
    geom_line(color = "steelblue", linewidth = 1) +
    geom_point(color = "darkred", size = 2) +
    geom_text(aes(label = count), vjust = -1.3, fontface = "bold", size = 4) +
    scale_y_continuous(limits = c(0, 60), breaks = seq(0, 60, 10)) +
    scale_x_date(
    breaks = seq(as.Date("2020-01-01"), as.Date("2027-01-01"), by ="1 year"),
    labels = date_format("%Y"),
    limits = c(as.Date("2020-01-01"), as.Date("2027-01-01"))
    ) +
    labs(
    title = title_plot,
    subtitle = subtitle_text,
...
## save plot
ggsave(
    plot = p_pet_vaccination,
    filename = "plot/pet_vaccination.jpeg",
    units = "in",
    width = 8,
    height = 4,
    dpi = 300
...
# display plot
knitr::include_graphics("plot/pet_vaccination.jpeg")
```

### Pet's living arrangement

- Nearly half of the pets are kept indoors (46%, n=58) while about one-third are kept in cages (32%, n=40), reflecting controlled environments for safety or management.

- A smaller portion are leashed (17%, n=22), very few are kept in fenced areas (3%, n=4). Only 2% (n=2) of pets were reported as stray, suggesting limited cases of unconfined animals.



```{r}

title_plot <- glue("Living arrangements of Pets (n = {nrow(housing_dta)})")

subtitle_text <- str_wrap("Most pets are kept indoors or caged, with fewer leashed, fenced, or stray.", 70)
...
p_pet_condition <- 
  ...
  na.omit() |> 
  mutate(pet_stat = str_to_lower(pet_stat)) |> 
  mutate(pet_condition = case_when(
    str_detect(pet_stat, "house|indoor|domestic") ~ "Indoor",
    str_detect(pet_stat, "cage|caged") ~ "Caged",
    str_detect(pet_stat, "leased|leash") ~ "Leashed",
    str_detect(pet_stat, "fence|perimeter|pen|pond|gate") ~ "Fenced",
    str_detect(pet_stat, "stray|outside") ~ "Stray",
    TRUE ~ pet_stat
  )) |> 
  plt_barplot(pet_condition, title_plot)

## save plot
ggsave(
    plot = p_pet_condition,
    filename = "plot/pet_condition.jpeg",
    units = "in",
    width = 8,
    height = 4,
    dpi = 300
...
# display plot
knitr::include_graphics("plot/pet_condition.jpeg")
```
